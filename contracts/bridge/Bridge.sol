...
contract Bridge is Initializable, DelegateCallAware, IBridge {
    ...
    function acceptFundsFromOldBridge() external payable {} //@audit 去掉payable关键字

    function gasToken() external view returns (IGasToken) {
        return _gasToken;
    }

    function setGasToken(IGasToken gasToken_) external {//@audit 抢跑风险
    ...
    function scaleBaseFeeL1(uint256 fee) internal view returns (uint256) {
        ...
        IPriceFeed _priceFeed = IPriceFeed(address(0x62CAe0FA2da220f43a51F86Db2EDb36DcA9A5A08));//@audit  这里还是测试网的地址，修改为arb主网的
