...
contract SequencerInbox is DelegateCallAware, GasRefundEnabled, ISequencerInbox {
    ...
    /// @inheritdoc ISequencerInbox
    function removeDelayAfterFork() external {//@audit 这里是不是要加个权限控制，只判断了是否fork,任何人可以反复重置maxTimeVariation的值
    ...
    function invalidateKeysetHash(bytes32 ksHash) external onlyRollupOwner {
        ...
        dasKeySetInfo[ksHash].isValidKeyset = false;//@audit-ok 这里设置为false,是不是又可以调用上面setValidKeyset,但是感觉是合理的
